...
int Change_Element_List(List* list, Element *pointer) {
    ...
    if (list == NULL || pointer == NULL || *list == NULL || pointer->next == NULL) {
        return 0;
    }

    Element *next = pointer->next;

    if (*list == pointer) { //Se for o primeiro da lista
...
/*
 * Não tem muito o que explicar no quesito lógica, foi apenas trocas de posições, algo básico, mas uma curiosidade é que
 * essa função só funciona se for o mesmo ponteiro que está na lista, já apontando para o elemento, se for um
 * outro ponteiro criado por fora, mas que também aponta para o mesmo elemento, não irá dar certo, por isso criei a função
 * alí para conseguir pegar esse ponteiro.
 */